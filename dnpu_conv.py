...
class DNPUAct(DNPU):
    def __init__(self, processor, data_input_indices = [[3]], forward_pass_type = 'vec'):
        super().__init__(processor, data_input_indices, forward_pass_type)
    ...
    def add_input_transform(self, input_range, strict = True):
        super(DNPUAct, self).add_input_transform(input_range, strict=strict)

    def preprocess(self, x):
        """"
        x:input -> Tensor: [batch_size, XX]
        """
        org_input_dim = x.size()
        x = x.flatten(1)
        x = x.unsqueeze(1).expand(x.size(0),
                                  self.get_data_input_electrode_no(), x.size(1))
        
        if self.input_transform:
            self.add_input_transform([0., 1.], True)

        return x, org_input_dim
    ...
    def merge_electrode_data(self, x):
        ...
        controls_shape.insert(1, x.shape[2])

        del controls_shape[2]
        ...
        indices = torch.argsort(torch.cat((input_indices.permute(0, 2, 1), control_indices),
                                          dim = last_dim),
                                dim = last_dim)
        
        data = torch.cat((x.permute(0, 2, 1), controls), dim=last_dim)
    ...
    def forward(self, x):
        x, org_input_dim = self.preprocess(x)
        x = self.merge_electrode_data(x)
        x = self.processor(x)
        
        return x.resize(org_input_dim)
    ...
    def forward(self, x):
        ...
        -------
        result: torch.Tensor
            post activation function. With a shape of (batch_size, ...)
        """
        x, orig_shape = self.preprocess(x)
        x = self.merge_electrode_data(x)
        x = self.processor(x)
        # x = self.postprocess(x, orig_shape)
...
class DNPUConv1D(DNPU):
    def __init__(
        ...
        super(DNPUConv1D, self).__init__(
            processor,
            data_input_indices,
    ...
    def add_input_transform(self, input_range, strict=True):
        ...
        """
        super(DNPUConv1D, self).add_input_transform(input_range, strict=strict)
